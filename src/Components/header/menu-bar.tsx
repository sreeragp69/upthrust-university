...
export function MenuBar() {
  return (
    ...
    >
      <motion.div
        className={`absolute -inset-2 bg-gradient-radial from-transparent ${"via-blue-400/30 via-60% via-purple-400/30 via-60% via-red-400/30 via-90%"} to-transparent rounded-3xl z-0 pointer-events-none`}
        ...
        {menuItems.map((item, index) => (
          <motion.li key={item.label} className="relative">
            ...
            >
              ...
              >
                <span
                  className={`transition-colors font-alexandria duration-300 group-hover:${item.iconColor} text-foreground`}
                >
                  {/* {item.icon} */}
                </span>
                <span className="font-alexandria uppercase xl:text-[11px] 2xl:text-xs">{item.label}</span>
              </motion.a>
              <motion.a
                href={item.href}
              ...
              >
                <span
                  className={`transition-colors duration-300 font-alexandria group-hover:${item.iconColor} text-foreground`}
                >
                  {/* {item.icon} */}
                </span>
                <span className="font-alexandria uppercase xl:text-[11px] 2xl:text-xs ">{item.label}</span>
